...
def tfidf(train_data1,test_data1):
	X=[];Y=[]
	for i in train_data1:
		X.append(' '.join(i))
	for i in test_data1:
	...
	return tf.fit_transform(X),tf.transform(Y)
...
def train(X, y):#, input_shape, output_shape,vocabulory):
	...
	#clf3=neural_network.MLPClassifier(hidden_layer_sizes=(10,))
	clf4=tree.DecisionTreeClassifier()
	clf5=RandomForestClassifier()
	clf6=BernoulliNB()
	anova_svm=VotingClassifier(estimators=[('c1',clf1),('c2',clf2),('c4',clf4),('c5',clf5),('c6',clf6)],voting='hard')
	anova_svm.fit(X, y)
...
if __name__ == '__main__':
	...
	train_X,test_X=tfidf(train_X1,test_X1)
	train_Y = np_utils.to_categorical(train_labels[:], labels)
	print '--- train X shape: --', train_X.shape
	print '--- train labels ---', train_Y.shape
	print '--- test X shape ---', test_X.shape
	model = train(train_X, train_labels)#, train_X.shape[1], labels,vocabulory)
	print "model trained"	
